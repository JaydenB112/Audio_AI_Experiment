"""
Step 4: add ElevenLabs TTS. Full loop.

Mic audio -> Deepgram STT -> Claude -> ElevenLabs TTS -> speaker. Talk into
the mic, Claude's reply prints to console and is also spoken back out loud.

Requires headphones or earbuds. Once TTS is playing out loud speakers while
the mic stays open, the mic will pick up Claude's own voice and can confuse
turn detection (the pipeline may think you're talking while it's still
speaking). Headphones prevent that the same way they fixed the step 1
feedback loop.
...
async def main():
    ...
    logger.info("Listening. Talk, then stop, Claude will reply out loud.")
