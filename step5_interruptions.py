"""
Step 5: verify interruption handling.

Same full loop as step 4: mic -> Deepgram STT -> Claude -> ElevenLabs TTS ->
speaker. Adds one thing: a console log of bot/user speaking state so you can
confirm, from the log and not just by ear, that starting to talk while
Claude is still speaking makes it stop and listen rather than talk over you.

Pipecat handles this natively once a VAD analyzer is wired into the user
context aggregator (already true as of step 3). This script doesn't add
interruption handling, it adds visibility into whether it's working.

To test: let Claude start speaking, then start talking part way through its
reply. Watch for "started speaking while bot was talking" in the log, and
confirm playback actually cuts off rather than continuing under your voice.
...
async def main():
    ...
    logger.info("Listening. Let Claude start replying, then talk over it to test interruption.")
